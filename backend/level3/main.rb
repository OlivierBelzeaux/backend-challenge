...
all_rentals.each_with_index do |rental, index|
  ...
  insurance_fee = amount_of_insurance(reduced_price)
  assistance_fee = amount_of_roadside_assistance(100, rental_days)
  drivy_fee = (reduced_price * 0.3) - (insurance_fee + assistance_fee)

  rentals["rentals"] << ({"id": (index + 1), "price": reduced_price, "commission": {"insurance_fee": insurance_fee.to_i, "assistance_fee": assistance_fee.to_i, "drivy_fee": drivy_fee.to_i}})
end
